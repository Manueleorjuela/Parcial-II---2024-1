...
bool Red_Metro::Validacion_Linea_Conectar_Ya_Conectada(string &Nombre_Linea, int Case, Linea &Linea, int& Posicion_Estacion)
{
    bool Validacion = false;

    if (Lineas_Conectadas != nullptr) {
        for (int i = 0; i < Cantidad_Conexiones; i++) {
            if (Nombre_Linea == Lineas_Conectadas[i]) {
                Validacion = true;
                if (Case == 2) {
                    Error7_LineaIngresadaNoHaceParteDeLaRed();
                    Linea.Eliminar_Estacion(Posicion_Estacion);
...
bool Red_Metro::Validacion_Conexiones(string *&Actualizacion)
{
    bool Validacion = false;
    if (Lineas_Conectadas == nullptr){
        Cantidad_Conexiones +=2;
        Validacion = true;
    }else Cantidad_Conexiones +=1;

    Actualizacion = new string[Cantidad_Conexiones];
    return Validacion;
}
        ...
        if(!Validacion_Error4(Posicion)){
            ...
            if(Transferencia){
                if(!Validacion_Error6(Posicion_Estacion, Metro[Posicion])){
                    ...
                    //VERIFICAR ERROR DE NO ELIMINAR EN A

                    if(Validacion_Error7(Nombre_LineaConectar, Posicion_Estacion, Metro[Posicion], Nombre_LineaActual)){

                       if(Validacion_Linea_Conectar_Ya_Conectada(Nombre_LineaActual, 1, Metro[Posicion_Estacion], Posicion_Estacion)){

                        if(!Validacion_Linea_Conectar_Ya_Conectada(Nombre_LineaConectar, 2, Metro[Posicion], Posicion_Estacion)){
                            ...
                            Agregar_Linea_A_Conectadas(Nombre_LineaActual, Nombre_LineaConectar, 2);
                            }
                        }else{

                            Posicion_Linea_Transferencia = Buscar_Posicion_Linea(Nombre_LineaConectar);
                            Metro[Posicion_Linea_Transferencia].Añadir_Estacion(Transferencia, Nombre_Estacion_Conexion);
                            Agregar_Linea_A_Conectadas(Nombre_LineaActual, Nombre_LineaConectar, 1);
                        }
                    }
                }
            }
...
void Red_Metro::Agregar_Linea_A_Conectadas(string &Nombre_LineaActual, string &Nombre_LineaConectar, int Case)
{
    string *Actualizacion;

    if (Validacion_Conexiones(Actualizacion)){
        Actualizacion[0] = Nombre_LineaActual;
        Actualizacion[1] = Nombre_LineaConectar;
    }else{
        copy(Lineas_Conectadas, Lineas_Conectadas + (Cantidad_Conexiones - 1), Actualizacion);
        switch (Case){
        case 1:
            Actualizacion[Cantidad_Conexiones-1] = Nombre_LineaActual;
            break;
        case 2:
            Actualizacion[Cantidad_Conexiones-1] = Nombre_LineaConectar;
        }
    }
...
{
    try {
        throw runtime_error("La linea ingresada para crear la transferencia no es valida, no se creara la estacion.");
    } catch (const runtime_error& ex) {
        cout << "Error: " << ex.what() << endl;
    }
}
